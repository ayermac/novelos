...
- **当前爽文感知基线**: v6.8.1 Webnovel Excitement Awareness
- **当前 Preflight UX 基线**: v6.7.3 Preflight UX & Regression Closure
- **当前创作工厂能力基线**: v6.9.0 Creative Factory Capability Upgrade
- **当前知识化基线**: v6.10.0 Skill Knowledge & LLM Function Calling
- **下一阶段 Skill 工程化规划**: v6.10.1 Skill Engineering Standardization
- **当前 Genesis 稳定性基线**: v6.10.6 Genesis Hardening
- **当前故事合同治理基线**: v6.10.7 Core Loop Evidence Governance
- **当前风格管理基线**: v6.10.4 Style Management Hardening
- **当前诊断稳定性基线**: v6.10.3 Workflow Diagnostics & Stability
- **下一阶段架构强化规划**: v6.10.13 Architecture Hardening
- **状态**: v6.10.13 借鉴 ainovel-cli 设计模式提升长篇创作可靠性：引入 FlowRouter 纯函数路由、SignalStore 一次性信号机制、StepCheckpoint Agent 内部断点、StopGuard 物理不可停机、BudgetSentinel 预算哨兵、StyleStats 纯代码风格统计、DiagnosisSystem 诊断系统、SteerManager 用户干预、Notifier 通知系统；v6.10.7 将 Story Contract 从提示治理升级为证据治理：核心循环门禁不再把 ChapterBrief 声明或泛关键词命中直接当作兑现，新增奖励获得、奖励使用、敌方反噬、必需卖点兑现、数值状态继承诊断，Creative Ledger 持久化 `evidence_spans`、`tracked_states`、`state_deltas` 和 `missing_evidence`，workflow timeline / run detail 暴露核心循环诊断卡片，核心循环阻断返修目标指向 author；v6.10.6 修复 Genesis 分段生成后的契约不一致问题：章节指令 prompt 恢复为结构化施工单，质量门优先读取 protagonist/primary_location/opposing_force/action_chain/visible_result/state_change 等字段，real-mode 创世在指令质量阻塞时会只返修 `instructions`，不重写世界观、角色、大纲或伏笔；同时创世分段 LLM 超时会保留已完成分段并本地补齐缺失部分，不再直接让整次初始化失败；v6.10.5 将创作合同从展示/审批升级为可执行 Story Contract Governance：项目生成 `story_contract`，章节 Brief 带核心兑现目标，planner/screenwriter/author/polisher/editor 注入故事合同，QualityGate 增加核心循环合规诊断，Creative Ledger 持久化合同指标，项目页可编辑核心承诺、核心循环、辅助机制和漂移规则；v6.10.4 将风格管理从"存在记录"升级为"用户可维护、系统可消费、生成链路真正生效"的项目级创作控制能力：风格初始化生成 canonical StyleBible、旧格式自动 normalize、新增 `GET/PUT /style/bible/{project_id}` 结构化 API、项目页编辑直接定位当前项目、全局 `/style` 页面支持查看/编辑/门禁配置、Author plain-text 路径通过 AgentContextBuilder 统一注入 Style Bible、Style Gate 默认 enabled=false 不增加阻塞率；v6.10.3 增加 Run Doctor 运行故障归因、QualityGate mandatory/advisory checker 健康分级、发布前标题硬校验、Memory Curator 降级发布就绪策略与补跑入口；v6.10.0 引入双层 Skill 架构（知识层 Markdown + 执行层 Python）、LLM Function Calling、实时事件队列（EventQueue）、正文流式输出、11 个知识 Skill；v6.9.0 引入创作合同系统（项目启动画像、类型合同、章节 Brief 合同）、节奏预算和创作台账追踪、8 视角专业编辑评审（type/continuity/commercial/pacing/character/mystery/style + chief editor），实现从项目启动到章节生产的全流程创作控制；v6.8.1 新增风格感知能力（style_detector.py 确定性检测、Planner/Screenwriter/Author/Editor prompt 注入、Editor pacing 权重 15→30、opening-hook-checker 和 excitement-density-checker Skills）；v6.8.0 将确定性质量检查注册为标准 Skills（continuity-gate, chapter-seam, death-penalty, word-count-gate, fact-lock），新增测试基础设施（test-impacted.py, test-smoke.sh, pytest markers）；v6.7.9 新增确定性叙事连续性硬门控（时间回退/跨章锚点/事件重播检测），Editor fallback 降权至 70 分；v6.7.8 修正 revision retry 会计（内部压缩不再消耗章节重试），新增状态型事实过滤器减少误报；v6.7.7 为 Genesis 项目设定生成新增异步启动和 SSE 分段进度展示；v6.7.6 修复 blocked/failed/stale-running 状态下 terminal chapter status 导致 recovery CTA 被 publish 掩盖的问题；v6.7.5 实现独立章节标题生成机制，移除 content opening 作为主要标题来源，使用 LLM 基于章节内容和指令上下文生成更有吸引力的标题；v6.7.3 完善 preflight 前端展示（非阻断式警告横幅）、增强 warning details（groups、ids、recommended_actions）、补齐 API 成功路径回归测试；v6.7.2 新增轻量级 preflight 诊断，在章节生成前暴露重复角色/世界观、记忆压力等问题。
- **版本规划索引**: [planning/novel-factory-version-planning-index.md](planning/novel-factory-version-planning-index.md)
- **v6.11.02 发布完整性与运行时收敛优化计划**: [planning/novel-factory-v6.11.02-release-integrity-runtime-convergence-plan.md](planning/novel-factory-v6.11.02-release-integrity-runtime-convergence-plan.md)
- **v6.11.01 架构债务优化规划**: [planning/novel-factory-v6.11.01-architecture-debt-optimization-plan.md](planning/novel-factory-v6.11.01-architecture-debt-optimization-plan.md)
- **v6.10.13 规划**: [planning/novel-factory-v6.10.13-architecture-hardening-plan.md](planning/novel-factory-v6.10.13-architecture-hardening-plan.md)
- **v6.10.7 规划**: [planning/novel-factory-v6.10.7-core-loop-evidence-governance-plan.md](planning/novel-factory-v6.10.7-core-loop-evidence-governance-plan.md)
- **v6.10.6 规划**: [planning/novel-factory-v6.10.6-genesis-hardening-plan.md](planning/novel-factory-v6.10.6-genesis-hardening-plan.md)
- **v6.10.5 规划**: [planning/novel-factory-v6.10.5-story-contract-governance-plan.md](planning/novel-factory-v6.10.5-story-contract-governance-plan.md)
- **v6.10.4 规划**: [planning/novel-factory-v6.10.4-style-management-hardening-plan.md](planning/novel-factory-v6.10.4-style-management-hardening-plan.md)
- **v6.10.3 规划**: [planning/novel-factory-v6.10.3-workflow-stability-recovery-plan.md](planning/novel-factory-v6.10.3-workflow-stability-recovery-plan.md)
- **v6.10.3 完成报告**: [reports/novel-factory-v6.10.3-completion-report.md](reports/novel-factory-v6.10.3-completion-report.md)
- **v6.10.3 Review 记录**: [reviews/novel-factory-v6.10.3-review.md](reviews/novel-factory-v6.10.3-review.md)
- **v6.10.1 Skill 工程化规划**: [planning/novel-factory-v6.10.1-skill-engineering-standardization-spec.md](planning/novel-factory-v6.10.1-skill-engineering-standardization-spec.md)
- **v6.10.0 开发提示**: [planning/novel-factory-v6.10.0-dev-prompt.md](planning/novel-factory-v6.10.0-dev-prompt.md)
- **v6.9.0 阶段计划**: [planning/novel-factory-v6.9.0-phase-plan.md](planning/novel-factory-v6.9.0-phase-plan.md)
- **v6.9.0 完成报告**: [reports/novel-factory-v6.9.0-completion-report.md](reports/novel-factory-v6.9.0-completion-report.md)
- **v6.8.1 规格**: [specs/novel-factory-v6.8.1-webnovel-excitement-awareness-spec.md](specs/novel-factory-v6.8.1-webnovel-excitement-awareness-spec.md)
- **v6.8.1 完成报告**: [reports/novel-factory-v6.8.1-completion-report.md](reports/novel-factory-v6.8.1-completion-report.md)
...
- **v6.7.8 完成报告**: [reports/novel-factory-v6.7.8-completion-report.md](reports/novel-factory-v6.7.8-completion-report.md)
