...
| v5.3.2 | Project Genesis & Memory Loop | 从一句创意生成项目骨架，并在章节后自动提出项目资料和事实更新 |
| v5.3.3 | Workflow Observability | Agent step log、产物明细、token/耗时、质量门详情、版本 diff、人工审核记录 |
| v5.3.4 | Continuity & Fact Ledger | 跨章事实/数值账本、状态继承、冲突检测、连续性发布门禁 |
...
详细规格：`novel-factory-v5.3.2-project-genesis-memory-loop-spec.md`

v5.3.2 优先补“创作记忆循环”：从一句创意生成项目骨架，并在后续章节生产后自动提出世界观、角色、势力、大纲、伏笔、章节指令和事实账本更新。该版本要求 AI 生成的是可审核 patch，而不是静默覆盖正式资料。
