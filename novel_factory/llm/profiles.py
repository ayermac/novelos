...
class LLMProfilesConfig(BaseModel):
    ...
    def get_fallback_profile_for_agent(self, agent_id: str) -> tuple[str, Optional[LLMProfile]]:
        """Get the fallback profile name and profile for an agent.
        ...
        Returns:
            Tuple of (profile_name, profile or None if not found).
        """
        profile_name = self.agent_llm_fallback.get(agent_id)
        if not profile_name:
            return "", None
        profile = self.llm_profiles.get(profile_name)
        return profile_name, profile
    ...
    def validate_profiles(self) -> list[str]:
        ...
        # Check that all agent_llm_fallback references exist
        for agent_id, profile_name in self.agent_llm_fallback.items():
            if profile_name not in self.llm_profiles:
                issues.append(
