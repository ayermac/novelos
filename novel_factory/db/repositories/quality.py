...
class QualityRepositoryMixin:
    ...
    # ── Editor Lens Reports (v6.9.0) ──────────────────────────

    def save_editor_lens_report(
        self,
        project_id: str,
        chapter_number: int,
        lens_type: str,
        ...
        """Save an editor lens report.

        Args:
            project_id: Project identifier.
            chapter_number: Chapter number.
            lens_type: Lens type (type, continuity, commercial, pacing, character, mystery, style).
        ...
        try:
            cursor = conn.execute(
                ...
                "(project_id, chapter_number, lens_type, report_data, workflow_run_id) "
                "VALUES (?, ?, ?, ?, ?)",
                (
                    project_id,
                    chapter_number,
    ...
    def get_editor_lens_reports(
        ...
        Args:
            project_id: Project identifier.
            chapter_number: Chapter number.
            lens_type: Optional lens type filter.

        Returns:
            List of editor lens reports.
        """
        conn = self._conn()
        try:
            query = "SELECT * FROM editor_lens_reports WHERE project_id=? AND chapter_number=?"
            ...
            for row in rows:
                ...
                if isinstance(report_data, str):
                    r["report_data"] = json.loads(report_data)
                results.append(r)
            return results
        finally:
            conn.close()

    def get_latest_editor_lens_report(
        self,
        project_id: str,
        chapter_number: int,
        lens_type: str,
    ) -> dict | None:
        """Get the latest editor lens report for a specific lens type.
        ...
        """
        reports = self.get_editor_lens_reports(project_id, chapter_number, lens_type)
        return reports[0] if reports else None
