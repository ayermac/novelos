-- 网文工厂数据库初始化脚本
-- 执行方式：sqlite3 shared/data/novel_factory.db < shared/data/init_db.sql
-- 创建日期: 2026-03-31
...
CREATE TABLE IF NOT EXISTS chapter_versions (
    ...
    created_by TEXT DEFAULT 'author',  -- 'author', 'revision', 'rollback'
    review_id INTEGER,  -- 关联质检报告
    notes TEXT,
    FOREIGN KEY (project_id) REFERENCES projects(project_id),
...
CREATE TABLE IF NOT EXISTS state_history (
    ...
    changed_fields TEXT,  -- JSON: {"field": {"from": old, "to": new}}
    reason TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (project_id) REFERENCES projects(project_id)
);
...
-- 注意：这是"异步消息队列"，Agent 不直接通知彼此
-- Dispatcher 在调度 Agent 时检查 pending messages
CREATE TABLE IF NOT EXISTS agent_messages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    project_id TEXT,  -- 关联项目，NULL 表示全局消息
    from_agent TEXT NOT NULL,
    to_agent TEXT NOT NULL,
    type TEXT NOT NULL,  -- FLAG_ISSUE, ESCALATE, SUGGEST, NOTIFY
    priority TEXT DEFAULT 'normal',  -- normal, high, urgent
    content TEXT NOT NULL,  -- JSON
    chapter_number INTEGER,  -- 关联章节
...
CREATE TABLE IF NOT EXISTS learned_patterns (
    ...
    project_id TEXT,
    chapter_number INTEGER,
    category TEXT NOT NULL,  -- 'ai_trace', 'logic', 'setting', 'poison', 'pacing'
    pattern TEXT NOT NULL,  -- 问题模式描述
...
-- 触发器：自动保存版本（当草稿更新时）
-- ============================================================
-- 注意：SQLite 触发器在表已存在时需要特殊处理
-- 建议在应用层（db_common.py save_draft）调用 VersionManager
